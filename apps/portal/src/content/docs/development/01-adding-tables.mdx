...
<Alert variant="info">
  Most contributors don't need to add tables. The existing tables cover spells,
  items, talents, and other common data.
</Alert>
...
```sql
-- 20260117000000_add_my_new_table.sql
...
```bash
supabase gen types typescript --schema game,raw_dbc > apps/portal/src/lib/supabase/database.types.ts
```
