...
ActionState {
    ...
    da_multiplier: f32,
    ta_multiplier: f32,
    persistent_multiplier: f32,
    player_multiplier: f32,
...
```
mitigation = min(armor / (armor + armor_constant), 0.85)
damage_after = damage * (1 - mitigation)
```

The armor constant is level-dependent (7390 for level 80 targets). Mitigation is capped at 85%.
...
```rust
DamageResult {
    raw: f32,           // Raw damage before mitigation
    final_amount: f32,  // Final damage after everything
...
- [Stats & Resources](/dev/docs/reference/04-stats-resources) - How stats are computed
- [Auras & Procs](/dev/docs/reference/05-auras-procs) - Buff/debuff mechanics
