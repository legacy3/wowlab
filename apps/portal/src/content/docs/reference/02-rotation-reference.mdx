...
Rotations compile to an abstract syntax tree. The AST uses string identifiers for readability:
...
// AST Action (parser output, uses string names)
...
After resolution, actions compile to a simpler runtime representation:

```rust
// Runtime Action (JIT output, uses indices)
Action::Cast(SpellIdx)
Action::WaitGcd
Action::Wait(f64)
Action::Pool { target: f64 }
Action::None
```

## Expression types

Expressions fall into these categories:

| Category   | Expressions                                                  |
| ---------- | ------------------------------------------------------------ |
| Literals   | `Bool`, `Int`, `Float`                                       |
| Resources  | `Current`, `Max`, `Deficit`, `Percent`, `Regen`, `TimeToMax` |
| Cooldowns  | `Ready`, `Remaining`, `Charges`, `ChargesFractional`         |
| Auras      | `Active`, `Stacks`, `Remaining`, `Refreshable`               |
| Combat     | `Time`, `Remaining`                                          |
| Target     | `Health`, `HealthPercent`, `TimeToDie`, `Distance`           |
| Player     | `Haste`, `Crit`, `Moving`                                    |
| Logical    | `And`, `Or`, `Not`                                           |
