...
Combat simulation requires millions of iterations to produce statistically meaningful results. A 5-minute fight at 1000 iterations means simulating 83+ hours of combat. Speed determines whether users wait seconds or minutes for results.
...
**Dynamic rotation loading:** Rotations could be interpreted at runtime instead of compiled. This would simplify the codebase. Rejected: 100x slowdown.
...
**HashMap for aura lookup:** Would simplify AuraTracker. Rejected: SmallVec with linear scan is faster for typical aura counts (stack-allocated for up to 16 auras).
...
| SmallVec auras | HashMap         | Slower for ≤16 auras    |

The engine is the result of iterative refinement. Each decision was tested against alternatives. The result is the minimal architecture that achieves the required performance.
