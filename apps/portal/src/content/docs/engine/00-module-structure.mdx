...
crates/engine/src/
  ...
  api/       - Public API types for external consumers
...
`StatsCollector` aggregates simulation runs. Calculates mean, std dev, percentiles. `DamageBreakdown` for per-spell analysis.
