...
/**
 * Helper to query Supabase and handle errors consistently
 */
const query = <T>(
...
/**
 * Query rows by a foreign key (e.g., SpellID)
 */
...
/**
 * Query a single row by a foreign key
 */
...
/**
 * Creates a SupabaseDbcService layer that queries the raw_dbc schema.
 * Uses Effect.Cache for frequently accessed lookup tables with 5-minute TTL.
 */
export const SupabaseDbcService = (
  ...
  Layer.effect(
    DbcService,
    Effect.gen(function* () {
      // Create caches for lookup tables (keyed by ID)
      ...
      // Create caches for spell/item tables (keyed by SpellID/ItemID)
      ...
      const service: DbcServiceInterface = {
        ...
        getSpellCastingRequirements: (spellId) => spellCastingRequirementsCache.get(spellId),
        ...
        getSpellEmpowerStages: (spellEmpowerId) => spellEmpowerStagesCache.get(spellEmpowerId),
        ...
        getSpellProcsPerMinuteMods: (spellProcsPerMinuteId) => spellProcsPerMinuteModsCache.get(spellProcsPerMinuteId),
