...
fn test_target_time_to_die() {
    ...
    assert_eq!(ttd_zero, crate::types::SimTime::MAX);
}
...
fn test_target_time_to_percent() {
    ...
    assert_eq!(ttp_below, crate::types::SimTime::ZERO);

    // With 0 DPS - should be MAX
    let ttp_zero = enemy.time_to_percent(30.0, 0.0);
    assert_eq!(ttp_zero, crate::types::SimTime::MAX);
}
...
fn test_enemy_count_returns_alive_count() {
    ...
    // Kill one enemy
    if let Some(enemy) = enemies.get_mut(crate::types::TargetIdx(0)) {
