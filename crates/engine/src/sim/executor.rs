...
impl<'a> DecisionContext<'a> {
    ...
    pub fn buff_up(&self, aura: wowlab_common::types::AuraIdx) -> bool {
        self.state.player.buffs.has(aura, self.now)
    }

    /// Get buff stacks
    pub fn buff_stacks(&self, aura: wowlab_common::types::AuraIdx) -> u8 {
        self.state.player.buffs.stacks(aura, self.now)
    }

    /// Check if cooldown is ready
    pub fn cooldown_ready(&self, spell: wowlab_common::types::SpellIdx) -> bool {
        self.state
            .player
            .cooldown(spell)
    ...
    pub fn cooldown_remaining(&self, spell: wowlab_common::types::SpellIdx) -> SimTime {
        self.state
            .player
            .cooldown(spell)
